...
const PORT           = 26000;
const PM2_NAME       = '7DaysToDie';
const TELNET_PORT    = 26060;
const TELNET_PASS    = 'eY4gLv94';
...
function startWaker() {
  ...
  waker = net.createServer(sock => {
    ...
    sock.once('data', chunk => {
      // Filtre : le client 7DTD envoie des données (les scanners purs ne le font pas)
      ...
      if (prev && now - prev < CONFIRM_WINDOW) {
        ...
        // Laisser la connexion ouverte 3s (le client voit "connexion en cours")
        // avant de couper proprement
        setTimeout(() => sock.destroy(), 3000);
        startServer();
      } else {
        _wkIps.set(ip, now);
        console.log('[autopause] 1er paquet de ' + ip + ', confirmation dans 30s...');
        // Couper après 3s pour que le client retry et déclenche la confirmation
        setTimeout(() => sock.destroy(), 3000);
